...
#     .run_commands("git clone https://github.com/FindDefinition/cumm && cd ./cumm && pip install -e .")
#     .run_commands("git clone https://github.com/traveller59/spconv && cd ./spconv && pip install -e .")
#     .pip_install(["nuscenes-devkit"])  # Some reqs not in requirements.txt
...
#     .copy_local_dir("../pcdet", "/root/pcdet") # For debugging purposes
# )
# =======
# pcdet_cuda_image = (
#     modal.Image.from_registry(f"nvidia/cuda:{tag}", add_python="3.11")
...
pcdet_cuda_image = (
    ...
    .copy_local_dir("../tools", "/OpenPCDet/tools") # Copy the tools folder to the container. This means you can change configs files and run them without having to commit the changes.
    .copy_local_dir("../pcdet", "/OpenPCDet/pcdet") # For debugging purposes
)
